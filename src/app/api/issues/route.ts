...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (status) {
      where.status = status as Prisma.IssueWhereInput['status'];
    }
    if (severity) {
      where.severity = severity as Prisma.IssueWhereInput['severity'];
    }
    if (fleet) {
    ...
    logger.error('Error fetching issues:', error instanceof Error ? error : undefined);
