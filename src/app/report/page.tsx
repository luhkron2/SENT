...
export default function ReportPage() {
  ...
  const { register, control, handleSubmit, formState: { errors }, setValue, watch, reset } = useForm<ReportForm>({
  ...
  // Auto-save hook
  const { load: loadDraft, clear: clearDraft, getTimestamp: getDraftTimestamp } = useFormAutosave({
    key: 'issue-report',
    data: formValues,
  ...
  useEffect(() => {
    ...
    if (savedDraft && timestamp) {
      ...
      if (hoursDiff < 24) {
        reset(savedDraft);
        toast.info(`Draft restored from ${savedDate.toLocaleTimeString()}`, {
          action: {
            label: 'Clear',
            onClick: () => {
              clearDraft();
  ...
  }, [draftRestored, loadDraft, getDraftTimestamp, reset, clearDraft]);
