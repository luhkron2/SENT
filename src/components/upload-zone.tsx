...
import { compressImage } from '@/lib/mobile-utils';
  ...
  const handleFiles = useCallback(
    async (newFiles: FileList | null) => {
      ...
      for (const file of validFiles) {
        if (file.type.startsWith('image/') && file.size > 500 * 1024) { // Compress images over 500KB
          try {
  ...
  return (
    <div className={cn('space-y-4', className)}>
      ...
      >
        ...
        <label
          htmlFor={inputId}
          className={cn("cursor-pointer flex flex-col items-center", isProcessing && "pointer-events-none opacity-50")}
        >
          <Upload className={cn("w-12 h-12 mb-4 text-muted-foreground", isProcessing && "animate-pulse")} />
          <p className="text-sm font-medium mb-1">
            {isProcessing ? 'Processing images...' : 'Drop files here or click to upload'}
          </p>
